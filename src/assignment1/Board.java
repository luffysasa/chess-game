...
public class Board {
    ...
    public List<Cell> getsuggestions(Cell fromcell){
        return getPossibleDestinations(fromcell);
    }
    ...
    public Cell hints(Cell fromcell, Board board){
        ...
        if(turn.equals(Piece.Type.MUSKETEER)){
            Cell res_tocell = possiblecells.get(scores.indexOf(max(scores)));
            return new Cell(res_tocell);

        }else {
            Cell res_tocell = possiblecells.get(scores.indexOf(min(scores)));
